...
interface ThreeJSRacingGameProps {
  lessonId?: number;
  subjectId?: string;
  lessonName?: string;
  onGameComplete?: () => void;
...
const ThreeJSRacingGame: React.FC<ThreeJSRacingGameProps> = ({ 
  lessonId,
  subjectId,
  lessonName,
  onGameComplete 
  ...
  // Keep latest dynamic values in a ref so the animation loop doesn't read stale closures
  useEffect(() => {
    stateRef.current = {
      playerPosition,
      aiPosition,
  ...
  const [obstacleCars, setObstacleCars] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    // Initial pixel ratio setup for mobile
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
  ...
  const createObstacleRocks = (scene: THREE.Scene) => {
    ...
    // DISABLED: No rocks/blocks on track for clean racing
    // Create empty group to maintain compatibility
    scene.add(rockGroup);
  ...
  const createObstacleCar = (scene: THREE.Scene, zPosition: number) => {
    ...
    // Create a simple obstacle car (different from player/AI cars)
    const bodyGeometry = new THREE.BoxGeometry(1.6, 0.5, 3.5);
    const bodyMaterial = new THREE.MeshLambertMaterial({ color: 0x666666 }); // Gray color
    ...
    const wheelPositions = [
      [-0.8, 0, 1.2], [0.8, 0, 1.2], [-0.8, 0, -1.2], [0.8, 0, -1.2]
    ];
    ...
    // Position the obstacle car
    carGroup.position.set(0, 0.5, zPosition);
  ...
  const createAICar = (scene: THREE.Scene) => {
    ...
    const bodyGeometry = new THREE.BoxGeometry(1.8, 0.6, 4);
    const bodyMaterial = new THREE.MeshLambertMaterial({ color: 0xFFFFFF }); // Deep pink
    const body = new THREE.Mesh(bodyGeometry, bodyMaterial);
    body.position.y = 0.3;
    body.castShadow = true;
    carGroup.add(body);
  ...
  const updateScene = () => {
      ...
      // Move road backwards to simulate forward movement - use ref to avoid stale state
      const baseSpeed = 6; // base scrolling speed
      const relativeLead = (pPos - aPos);
      const speedFromProgress = relativeLead * 0.15; // emphasize player lead
      const boostBonus = boostVal > 30 ? 2 : 0;
      // Slow track during penalty to make failures obvious
      const underPenalty = !!(penaltyUntil && Date.now() < penaltyUntil);
      const penaltyFactor = underPenalty ? 0.6 : 1.0;
      const roadSpeed = Math.max(4, (baseSpeed + speedFromProgress + boostBonus) * penaltyFactor);
    ...
    if (roadRef.current) {
      // Short cycle keeps motion obvious while long road geometry hides seams
      roadRef.current.position.z = (nextOffset % 40) - 20;
    }
    
    // Update obstacle cars movement
    obstacleCarsRef.current.forEach((car, index) => {
      if (car) {
        // Move obstacle cars towards the player
        car.position.z += 0.5;
    ...
    // Spawn new obstacle cars every 5-6 cars passed
    if (carsPassed > 0 && carsPassed % 5 === 0 && !showQuestionPrompt) {
      if (sceneRef.current) {
        createObstacleCar(sceneRef.current, -30);
        setShowQuestionPrompt(true);
      }
    }
    
    // Check for collisions between player and obstacle cars
    if (playerCarRef.current && !collisionDetected) {
      ...
      obstacleCarsRef.current.forEach((obstacleCar) => {
        if (obstacleCar) {
          ...
          // Collision detection (adjust threshold as needed)
          if (distance < 2.5) {
            setCollisionDetected(true);
            
            // If no question was answered correctly, reduce lives
            if (!showQuestionPrompt) {
              // This will be handled by the game state when lives are reduced
    ...
    // Rocks are disabled, so no rock movement needed
    
    // Update car positions based on game state with continuous movement
    if (playerCarRef.current) {
      // Keep cars in their lanes but allow some movement
      playerCarRef.current.position.x = 3;
      
      // Visualize relative lead directly so correct/wrong answer jumps are obvious
      const relativeLead = (pPos - aPos);
      const playerZ = 2 - relativeLead * 0.25; // gentler mapping
      playerCarRef.current.position.z = clamp(playerZ, -15, 15);
    ...
    if (aiCarRef.current) {
      // AI car movement with continuous progression
      aiCarRef.current.position.x = -3;
      
      // Mirror relative lead so AI moves opposite of player
      const relativeLead = (pPos - aPos);
      const aiZ = 2 + relativeLead * 0.25; // gentler mapping
      aiCarRef.current.position.z = clamp(aiZ, -15, 15);
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (isCorrect) {
      // Correct answer: Add boost effect and avoid collision
      setRockBreaking(false); // No rocks to break
      setShowQuestionPrompt(false);
      setCollisionDetected(false);
      setCarsPassed(prev => prev + 1);
      
      // Automatically avoid the obstacle car by moving it out of the way
      obstacleCarsRef.current.forEach((obstacleCar) => {
        if (obstacleCar) {
          // Move obstacle car to the side to avoid collision
          obstacleCar.position.x = Math.random() > 0.5 ? 8 : -8;
        }
      });
      
      // Add visual feedback for correct answer
      if (playerCarRef.current) {
        // Brief speed boost effect
        const originalScale = playerCarRef.current.scale.x;
        playerCarRef.current.scale.setScalar(originalScale * 1.1);
        setTimeout(() => {
          if (playerCarRef.current) {
            playerCarRef.current.scale.setScalar(originalScale);
          }
        }, 300);
      }
    } else {
      // Wrong answer: Reduce boost and stagger, allow collision
  ...
  useEffect(() => {
    if (sceneRef.current && isPlaying) {
      // Spawn first obstacle car after a short delay
  ...
  // Update cars passed counter based on player position
  useEffect(() => {
    if (playerPosition > 0) {
      const newCarsPassed = Math.floor(playerPosition / 20); // Every 20 position units = 1 car passed
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="pointer-events-auto">
          ...
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2">
            <div className="bg-red-500 text-white px-3 py-1 sm:px-4 sm:py-2 rounded-lg font-bold text-base sm:text-lg shadow-lg" style={{clipPath: 'polygon(0 0, calc(100% - 20px) 0, 100% 100%, 20px 100%)'}}>
              {formattedTime}
            </div>
          </div>

          {/* Progress indicator with modern styling */}
          <div className="absolute top-12 sm:top-16 left-1/2 transform -translate-x-1/2 bg-white/10 backdrop-blur-md rounded-xl px-3 py-1 sm:px-4 sm:py-2 shadow-lg border border-white/20">
            <div className="text-xs sm:text-sm font-medium text-white text-center">
              Question {questionsAnswered + 1}/{maxQuestions}
            </div>
            <div className="text-xs text-blue-200 text-center hidden sm:block">
        ...
        {isLoading && (
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-50 pointer-events-auto">
            <div className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-lg p-4 sm:p-6 shadow-lg text-center mx-4">
              <div className="text-base sm:text-lg font-bold text-white mb-2">🏎️ Loading Race...</div>
              <div className="w-24 sm:w-32 h-2 bg-purple-200 rounded-full overflow-hidden">
        ...
        {answerFeedback?.show && (
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-40 pointer-events-none">
            <div className={`p-4 sm:p-8 rounded-2xl shadow-2xl text-center transform animate-bounce mx-4 ${
              ...
              <div className="text-xl sm:text-2xl font-bold text-white mb-2 sm:mb-4">
                {answerFeedback.isCorrect ? 'Correct!' : 'Wrong Answer!'}
              </div>
              {!answerFeedback.isCorrect && (
                <div className="text-sm sm:text-lg text-white">
                  <div className="mb-2">You chose: <span className="bg-red-700 px-2 py-1 sm:px-3 sm:py-1 rounded text-xs sm:text-sm">{answerFeedback.selectedAnswer}</span></div>
                  <div className="hidden sm:block">Correct answer: <span className="bg-green-600 px-3 py-1 rounded">{answerFeedback.correctAnswer}</span></div>
                </div>
              )}
            </div>
          </div>
        ...
        {isPlaying && currentQuestion && !isLoading && !answerFeedback?.show && (
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 pointer-events-auto px-4">
            <div className="bg-white/10 backdrop-blur-md border border-white/20 shadow-2xl rounded-2xl p-4 sm:p-8 w-full max-w-[90vw] sm:w-[600px] text-white text-center relative">
              {/* Category label */}
              <div className="text-xs sm:text-sm opacity-70 mb-2 text-blue-200 hidden sm:block">
                {currentQuestion.category || "Math"}
              </div>
              
              {/* Question Text */}
              <div className="text-lg sm:text-2xl font-bold mb-4 sm:mb-6 min-h-[40px] sm:min-h-[50px] leading-relaxed">
                {currentQuestion.question}
              </div>
              
              {/* Answer Options - vertical on mobile, horizontal on desktop */}
              <div className="flex flex-col sm:flex-row sm:flex-wrap justify-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                {currentQuestion.options.map((option, index) => (
                  <button
                    key={index}
                    className="bg-gradient-to-r from-orange-500 to-yellow-500 hover:from-orange-600 hover:to-yellow-600 text-white px-6 py-3 sm:px-8 sm:py-4 rounded-full text-lg sm:text-xl font-semibold transition-all duration-200 hover:scale-105 hover:-translate-y-1 min-w-[120px] shadow-lg touch-manipulation"
        ...
        {/* Obstacle Warning - appears when approaching obstacle cars */}
        {showQuestionPrompt && !answerFeedback?.show && (
          <div className="absolute top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 pointer-events-auto px-4">
            <div className="bg-red-500/90 backdrop-blur-md border-4 border-red-300 shadow-2xl rounded-2xl p-6 sm:p-8 text-white text-center animate-pulse">
        ...
        {showSaveMe && (
          <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-60 pointer-events-auto">
            <div className="bg-gradient-to-br from-purple-600 to-pink-600 rounded-2xl p-4 sm:p-8 shadow-2xl text-center max-w-sm sm:max-w-lg mx-4 border-4 border-yellow-400">
              <div className="text-3xl sm:text-4xl mb-3 sm:mb-4">🆘</div>
              <div className="text-lg sm:text-2xl font-bold text-white mb-3 sm:mb-4">Game Over! Need Help?</div>
              <div className="text-sm sm:text-lg text-yellow-100 mb-4 sm:mb-6">
                I noticed you struggled with some questions. Let me explain the concepts and help you learn!
              </div>
              <div className="text-xs sm:text-sm text-yellow-200 mb-4 sm:mb-6">
                Score: {score} | Time: {formattedTime} | Questions: {questionsAnswered}/{maxQuestions}
              </div>
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                <button
                  onClick={handleSaveMe}
                  className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 sm:px-6 sm:py-3 rounded-xl font-bold text-sm sm:text-lg transition-colors shadow-lg touch-manipulation"
                >
                  🎓 Ask Zuzuplay for Help!
                </button>
                <button
                  onClick={handleNoThanks}
                  className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 sm:px-6 sm:py-3 rounded-xl font-bold text-sm sm:text-lg transition-colors shadow-lg touch-manipulation"
                >
                  No Thanks
                </button>
              </div>
              <div className="text-xs text-yellow-200 mt-3 sm:mt-4 opacity-75 hidden sm:block">
        ...
        {!isPlaying && !isLoading && !showSaveMe && (
          <div className="absolute inset-0 bg-black/70 flex items-center justify-center z-50 pointer-events-auto">
            <div className="bg-gradient-to-br from-purple-800 via-purple-600 to-blue-600 rounded-2xl p-4 sm:p-8 shadow-2xl text-center border border-purple-400 mx-4">
              <h2 className="text-2xl sm:text-4xl font-bold text-white mb-3 sm:mb-4">
                🏎️ {lives > 0 ? "Victory!" : "Try Again!"} 🏁
              </h2>
              <div className="text-sm sm:text-lg text-purple-100 mb-4 sm:mb-6">
                Final Score: {score} | Time: {formattedTime}
              </div>
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <button
                  onClick={() => window.location.reload()}
                  className="bg-gradient-to-r from-orange-500 to-yellow-500 text-white px-6 py-2 sm:px-8 sm:py-3 rounded-xl hover:from-orange-600 hover:to-yellow-600 transition-all font-bold text-sm sm:text-lg shadow-lg touch-manipulation"
                >
                  🏎️ Race Again
                </button>
                {onGameComplete && (
                  <button
                    ...
                    className="bg-gradient-to-r from-blue-500 to-purple-500 text-white px-6 py-2 sm:px-8 sm:py-3 rounded-xl hover:from-blue-600 hover:to-purple-600 transition-all font-bold text-sm sm:text-lg shadow-lg touch-manipulation"
                  >
                    📚 Back to Lessons
                  </button>
                )}
        ...
        {/* Racing game status indicator - hidden on mobile, visible on desktop */}
        <div className="absolute bottom-4 right-4 bg-black/20 backdrop-blur-sm rounded-lg p-3 text-white text-xs pointer-events-none hidden lg:block">
